...
export function buildTemplatePreviewDocument(
  ...
  if (!['http:', 'https:'].includes(origin.protocol))
    throw new Error(
      'Preview links require an HTTP or HTTPS application origin.',
    )
  // Resolve outside srcdoc so client-side route changes cannot alter the handoff.
  const route = escapeAttribute(
    new URL(
      `/dashboard/requests/new?template=${encodeURIComponent(template.slug)}`,
      origin.origin,
    ).href,
  )
  const scenario = scenarioFor(template)
  const screenshots = [template.previewImage, ...template.screenshots].slice(
    0,
    3,
  )
  const featureCards = template.features
    .slice(0, 4)
    .map((feature) => `<article>${escapeHtml(feature)}</article>`)
    .join('')
  const pageCards = template.pages
    .slice(0, 5)
    .map((page) => `<li>${escapeHtml(page)}</li>`)
    .join('')
  const stack = template.techStack
    .slice(0, 4)
    .map((tech) => `<span>${escapeHtml(tech)}</span>`)
    .join('')
  const workflowCards = scenario.workflows
    .map(
      (item) =>
        `<article><b>${escapeHtml(item.title)}</b><span>${escapeHtml(item.detail)}</span></article>`,
    )
    .join('')
  const adminCards = scenario.admin
    .map((item) => `<li>${escapeHtml(item)}</li>`)
    .join('')
  const gallery = screenshots
    .map(
      (screenshot, index) =>
        `<img src="${escapeAttribute(screenshot)}" alt="${escapeAttribute(template.name)} screen ${index + 1}">`,
    )
    .join('')
      ...
      <body>
        ...
        <section id="sections" class="split">
          ...
          <div class="booking">
            ...
            <a class="cta" href="${route}" target="_top">Continue in client app</a>
          </div>
        </section>
        <section class="deep">
          <div>
...
function scenarioFor(template: PreviewTemplate) {
  ...
  const shared = {
    ...
    sectionCopy:
      'Service proof, conversion paths, contact capture, responsive sections, and Firebase-ready hooks can all travel with this template.',
    workflowHeadline: 'A real customer journey, not only a pretty page.',
    workflowCopy:
      'The public site, private forms, and admin-facing content areas can be shaped into a practical prototype before AuraFlow builds the final system.',
    adminCopy:
      'AuraFlow can turn this template into a managed workspace where owners update content, review leads, upload assets, and track requests without coding.',
    workflows: [
      {
        title: 'Capture interest',
        detail:
          'Visitors see a clear offer and continue into the private AuraFlow workspace for details.',
      },
      {
        title: 'Collect assets',
        detail:
          'Photos, brand notes, reference links, and documents can be attached to the build request.',
      },
      {
        title: 'Review previews',
        detail:
          'AuraFlow uploads progress previews for the client to approve or revise.',
      },
      {
        title: 'Launch and iterate',
        detail:
          'The finished build can start on a managed link or move to a custom domain.',
      },
    ],
    admin: [
      'Content editor',
  ...
  if (
    ...
    return {
      ...
      sectionCopy:
        'Menus can highlight jollof, waakye, fufu, banku, pastries, catering trays, drinks, and real restaurant photography without mixing unrelated images.',
      workflowHeadline: 'From appetite to order request.',
      workflowCopy:
        'A customer can inspect dishes, choose a branch or table time, request delivery, and continue the serious brief inside the client app.',
      workflows: [
        {
          title: 'Menu discovery',
          detail:
            'Dish cards, prices, dietary notes, and gallery sections make the food the star.',
        },
        {
          title: 'Booking flow',
          detail:
            'Visitors can request tables, catering, private dining, or event service.',
        },
        {
          title: 'Kitchen handoff',
          detail:
            'AuraFlow can wire order queues, availability, and WhatsApp confirmation for staff.',
        },
        {
          title: 'Seasonal updates',
          detail:
            'Admins can adjust menu items, specials, opening hours, and gallery photos.',
        },
      ],
      admin: [
        'Menu builder',
  ...
  if (
    ...
    return {
      ...
      sectionCopy:
        'Admissions, parents, students, teachers, timetables, fees, results, announcements, and document uploads can start from one configurable school template.',
      workflowHeadline: 'A school-management prototype path.',
      workflowCopy:
        'The template can become a low-cost hosted school system with role-based dashboards for administrators, teachers, parents, and students.',
      workflows: [
        {
          title: 'Admission enquiry',
          detail:
            'Parents submit enquiries and documents through structured school forms.',
        },
        {
          title: 'Parent portal',
          detail:
            'Families can receive announcements, fee updates, results, and term documents.',
        },
        {
          title: 'Teacher workspace',
          detail:
            'Staff workflows can include classes, attendance, reports, and notes.',
        },
        {
          title: 'Admin control',
          detail:
            'School owners manage terms, users, payments, and published updates.',
        },
      ],
      admin: [
        'Student records',
  ...
  if (
    ...
    return {
      ...
      sectionHeadline:
        'Show rooms clearly and turn visitors into booking requests.',
      sectionCopy:
        'Hotels, lodges, resorts, event halls, and guest houses can showcase rooms, amenities, pricing, local attractions, and booking status.',
      workflowHeadline: 'A guest journey from room search to confirmation.',
      workflowCopy:
        'AuraFlow can connect room galleries, availability requests, guest details, and admin follow-up into one hosted hospitality platform.',
      workflows: [
        {
          title: 'Room browsing',
          detail:
            'Guests compare rooms, amenities, rates, and real venue photography.',
        },
        {
          title: 'Booking request',
          detail:
            'Arrival date, nights, guests, and special notes enter one request flow.',
        },
        {
          title: 'Reception board',
          detail:
            'Staff can review incoming bookings and update confirmation status.',
        },
        {
          title: 'Gallery updates',
          detail:
            'Admins can publish new rooms, halls, pool photos, and seasonal offers.',
        },
      ],
      admin: [
        'Room editor',
  ...
  if (
    ...
    return {
      ...
      sectionCopy:
        'Departments, doctor profiles, service pages, appointment forms, prescription uploads, and patient follow-up can sit behind clear privacy boundaries.',
      workflowHeadline: 'From patient need to admin review.',
      workflowCopy:
        'The prototype can map patient intake, appointment assignment, document review, and pharmacy or clinic follow-up.',
      workflows: [
        {
          title: 'Service discovery',
          detail:
            'Patients find departments, opening hours, accepted requests, and care instructions.',
        },
        {
          title: 'Appointment intake',
          detail:
            'Structured forms collect symptoms, preferred dates, and contact details.',
        },
        {
          title: 'Document uploads',
          detail:
            'Prescriptions, lab references, or ID documents can be attached privately.',
        },
        {
          title: 'Admin triage',
          detail:
            'Reception or pharmacy staff review requests and update client-visible status.',
        },
      ],
      admin: [
        'Appointment board',
  ...
  if (
    ...
    return {
      ...
      sectionHeadline:
        'A shopfront that can grow into a managed commerce system.',
      sectionCopy:
        'Product catalogs, categories, stock, offers, checkout requests, delivery notes, and seller dashboards can be prototyped before the build.',
      workflowHeadline: 'From product browsing to order tracking.',
      workflowCopy:
        'AuraFlow can shape the flow for local shops, supermarkets, malls, fashion brands, electronics stores, and specialty retailers.',
      workflows: [
        {
          title: 'Browse products',
          detail:
            'Customers filter categories, inspect images, and compare prices.',
        },
        {
          title: 'Request checkout',
          detail:
            'Orders can begin as a managed request flow before online payments are added.',
        },
        {
          title: 'Inventory updates',
          detail:
            'Owners can update stock, promotions, departments, and delivery areas.',
        },
        {
          title: 'Sales visibility',
          detail:
            'Dashboards can summarize orders, best products, and fulfilment status.',
        },
      ],
      admin: [
        'Product editor',
  ...
  if (
    ...
    return {
      ...
      sectionCopy:
        'Practice areas, service packages, client onboarding, document collection, appointments, and progress updates can become a secure client portal.',
      workflowHeadline: 'From enquiry to managed client work.',
      workflowCopy:
        'The preview can become a private intake system where clients submit details and the business updates progress.',
      workflows: [
        {
          title: 'Qualified enquiry',
          detail:
            'Visitors select a service and provide enough context for a serious response.',
        },
        {
          title: 'Document request',
          detail:
            'Clients upload references, records, briefs, or forms inside their account.',
        },
        {
          title: 'Progress status',
          detail:
            'Admins share updates, deadlines, next steps, and review files.',
        },
        {
          title: 'Relationship history',
          detail:
            'Messages and revisions stay connected to the client request.',
        },
      ],
      admin: [
        'Lead inbox',
