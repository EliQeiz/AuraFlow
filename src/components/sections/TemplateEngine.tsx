...
export function TemplateEngine() {
  ...
  const active =
    categories.find((template) => template.slug === activeSlug) ?? categories[0]
  const srcDoc = useMemo(
    () => buildTemplatePreviewDocument(active, window.location.origin),
    [active],
  )
  const useLink = user
    ? `/dashboard/requests/new?template=${active.slug}`
    : '/register'
      ...
      <div className="section-shell">
        <div className="max-w-4xl">
          <h2 className="text-3xl font-extrabold sm:text-5xl">
            Templates grounded in Ghanaian and African business contexts.
          </h2>
          <p className="mt-4 text-lg text-aura-muted">
            Switch industries to preview food, hospitality, service, retail, and
            product directions that can be localized in the client app.
          </p>
        </div>
        <div className="mt-7">
          <Swiper
              ...
              <SwiperSlide key={template.slug}>
                <button
                  ...
                  className={`w-full rounded-lg border px-3 py-4 text-left font-bold transition ${
                    active.slug === template.slug
                      ? 'border-cyan-100 bg-cyan-300/15 text-white'
                      : 'border-white/10 bg-white/[0.06] text-aura-muted'
                  }`}
                >
                  {template.category}
            ...
            <aside className="border-b border-white/10 bg-black/25 p-5 lg:border-b-0 lg:border-r">
              ...
              <div className="mt-5 grid gap-3">
                <PreviewList
                  ...
                  items={active.pages.slice(0, 5)}
                />
                <PreviewList
                  title="Built-in functions"
                  items={active.features.slice(0, 5)}
                />
              </div>
              <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
                <ButtonLink
                  to={`/templates/${active.slug}`}
                  variant="secondary"
                >
            ...
            <div className="overflow-hidden bg-black/30 p-3 sm:p-5">
              <div className="overflow-hidden rounded-md border border-white/10">
                ...
                <iframe
                  ...
                  srcDoc={srcDoc}
                  className="h-[36rem] w-full bg-aura-dark"
                />
              </div>
            </div>
          </div>
    ...
    <div className="rounded-lg border border-white/10 bg-white/[0.05] p-3">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {items.map((item) => (
          <span
            key={item}
            className="rounded-md border border-cyan-200/20 bg-cyan-300/10 px-2 py-1 text-xs text-cyan-50"
          >
            {item}
          </span>
        ))}
