  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const request = await createProjectRequest({
        ...
        solutionSlug: selectedSolution?.slug,
        platformMode: selectedSolution ? 'managed-hosted' : 'custom-build',
        subdomainPreference: selectedSolution ? String(data.get('subdomainPreference') ?? '').trim() : undefined,
      })
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1fr_320px]">
      <Card className="p-4 sm:p-6">
        ...
        <form onSubmit={submit} className="mt-6 grid gap-4">
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Project title"><Input name="title" required defaultValue={selectedSolution ? selectedSolution.title : ''} placeholder="Accra food delivery app" /></Field>
            <Field label="Project type">
              <Select name="projectType" defaultValue={selectedSolution?.title ?? searchParams.get('service') ?? ''} required>
            ...
            <Field label="Target audience"><Input name="audience" required defaultValue={selectedSolution?.audience ?? ''} placeholder="Customers, staff, partners..." /></Field>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <Field label="Working budget in USD"><Input name="budget" type="number" min={39} max={100000} defaultValue={selectedSolution ? 499 : 2500} required /></Field>
            <Field label="Preferred hosted link">
              <Input name="subdomainPreference" defaultValue={selectedSolution?.platformLabel ?? ''} placeholder="yourbusiness.auraflow.app" />
            </Field>
          ...
          <label className="grid gap-2 rounded-lg border border-dashed border-cyan-200/35 bg-cyan-300/10 p-4 text-sm text-aura-muted">
            ...
            <span>Images or PDF files, up to 8 files and 10MB each. Add food photos, properties, people, screenshots, sketches, branding, or content files.</span>
      ...
      <div className="grid content-start gap-4">
        ...
        {selectedSolution ? (
          <Card className="p-5">
            <ServerSummary solutionSlug={selectedSolution.slug} title={selectedSolution.title} modules={selectedSolution.modules.slice(0, 5)} />
          </Card>
        ) : null}
        <Card className="p-5">
