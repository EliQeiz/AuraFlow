...
for ($index = 0; $index -lt $scenes.Count; $index++) {
  ...
  $durations += $duration
  $captionSentences = @($scene.Narration -split '(?<=[.!?])\s+' | Where-Object { $_.Trim() })
...
& ffmpeg -hide_banner -loglevel error -y -f concat -safe 0 -i $concatPath -c copy -movflags +faststart $finalVideo
...
$durationMinutes = [Math]::Round(($durations | Measure-Object -Sum).Sum / 60, 2)
Write-Host "Rendered $finalVideo ($durationMinutes minutes)"
