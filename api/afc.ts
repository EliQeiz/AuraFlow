...
import { supabaseAfcCommand } from '../server/afc/supabase-service.js'
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (serverBackendProvider === 'supabase') {
      ...
      const subjectHash = createHash('sha256').update(actor.uid).digest('hex')
      const { data: accepted, error: quotaError } = await db.rpc('consume_api_rate_limit', {
        p_scope: 'afc', p_subject_hash: subjectHash, p_max_requests: 60,
      })
      if (quotaError) throw new BusinessError(503, 'AFC protection services are unavailable.')
      if (accepted !== true) throw new BusinessError(429, 'Too many AFC requests. Please wait a minute.')
      res.status(200).json(await supabaseAfcCommand(db, actor, body))
      return
    }
